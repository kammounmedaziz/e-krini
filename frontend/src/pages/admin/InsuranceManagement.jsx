...
const InsuranceManagement = () => {
  ...
  const fetchCompanies = async (page = 1) => {
    try {
      setLoading(true);
      const response = await adminAPI.getAllInsurance({
        page,
  ...
  const handleAction = async () => {
    try {
      if (actionType === 'approve') {
        await adminAPI.approveInsurance(selectedCompany._id, actionReason);
        toast.success('Insurance company approved successfully');
      } else if (actionType === 'reject') {
        if (!actionReason.trim()) {
          toast.error('Rejection reason is required');
          return;
        }
        await adminAPI.rejectInsurance(selectedCompany._id, actionReason);
        toast.success('Insurance company rejected successfully');
      } else if (actionType === 'suspend') {
        if (!actionReason.trim()) {
          toast.error('Suspension reason is required');
          return;
        }
        await adminAPI.suspendInsurance(selectedCompany._id, actionReason);
        toast.success('Insurance company suspended successfully');
      } else if (actionType === 'delete') {
        await adminAPI.deleteInsurance(selectedCompany._id);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-white">Insurance Management</h1>
        <button
          onClick={() => fetchCompanies(pagination.currentPage)}
          disabled={loading}
      ...
      <div className="backdrop-blur-lg bg-white/10 border border-white/20 rounded-lg overflow-hidden">
        {loading ? (
          <div className="p-12 text-center">
            ...
            <p className="text-gray-300">Loading insurance companies...</p>
          </div>
        ) : companies.length === 0 ? (
          <div className="p-12 text-center">
            <HeartPulse className="w-12 h-12 mx-auto mb-4 text-gray-400" />
            <p className="text-gray-300">No insurance companies found</p>
          </div>
        ) : (
                  ...
                  <tr key={company._id} className="hover:bg-white/5 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        {company.logo ? (
                          <img src={company.logo} alt={company.companyName} className="w-10 h-10 rounded-full object-cover" />
                        ) : (
                    ...
                    <td className="px-6 py-4">
                      <p className="text-white text-sm">{company.address?.city || 'N/A'}</p>
                      <p className="text-gray-400 text-sm">{company.address?.country || 'N/A'}</p>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {company.status === 'approved' && (
                          <button
                            onClick={() => openActionModal(company, 'suspend')}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-gray-900 border border-white/20 rounded-lg p-6 max-w-md w-full">
            <h3 className="text-xl font-bold text-white mb-4">
              {actionType === 'approve' && 'Approve Insurance Company'}
              {actionType === 'reject' && 'Reject Insurance Company'}
              {actionType === 'suspend' && 'Suspend Insurance Company'}
              {actionType === 'delete' && 'Delete Insurance Company'}
            </h3>
            <p className="text-gray-300 mb-4">
              Are you sure you want to {actionType} <span className="font-semibold">{selectedCompany?.companyName}</span>?
            </p>
